...
const SoraProStudio: React.FC<SoraProStudioProps> = ({ userProfile, onProfileRefresh }) => {
    // Use per-model limit for Sora 2 Pro
    const modelLimit = userProfile?.sora2pro_limit ?? 0;
    const modelUsed = userProfile?.sora2pro_used ?? 0;
                    ...
                    <div className="mb-4 p-4 rounded-xl bg-amber-500/10 border border-amber-500/30">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex-1">
                                <h3 className="text-sm font-bold text-amber-500">Had Sora 2 Pro Telah Dicapai</h3>
                                <p className="text-xs text-muted-foreground">{modelUsed}/{modelLimit} video Sora 2 Pro digunakan. Hubungi admin untuk tambahan.</p>
                            </div>
                            <a
                                href={`https://wa.me/601158833804?text=${encodeURIComponent(`Hai Admin, saya ${userProfile?.username || 'user'} ingin mohon tambahan had Sora 2 Pro. Had saya: ${modelUsed}/${modelLimit}`)}`}
                                target="_blank"
